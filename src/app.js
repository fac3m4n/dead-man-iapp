...
const main = async () => {
  ...
  try {
    ...
    try {
      ...
      const protectedArticle = await deserializer.getValue("article", "string");
      const protectedRevealMethod = await deserializer.getValue(
        "revealMethod",
        "string"
      );
      const protectedRevealToken = await deserializer.getValue(
        "revealToken",
        "string"
      );
